...
class hico():
    def __init__(self, annotation_file, train_annotation):
        self.annotations = mmcv.load(annotation_file)
        self.train_annotations = mmcv.load(train_annotation)
        ...
        if len(pred_hoi) != 0:
            for i, pred_hoi_i in enumerate(pred_hoi):
                ...
                if triplet not in self.verb_name_dict:
                    continue
                verb_id = self.verb_name_dict.index(triplet)
                self.fp.setdefault(verb_id, [])
                self.tp.setdefault(verb_id, [])
                self.score.setdefault(verb_id, [])
