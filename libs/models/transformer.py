...
class InteractionTransformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, rel_query_embed, pos_embed):
        ...
        return rel_hs.transpose(1, 2), hs.transpose(1, 2), memory.permute(
            1, 2, 0).view(bs, c, h, w)
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_encoder_layers=6,
        ...
        rel_decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                    dropout, activation, normalize_before)
        rel_decoder_norm = nn.LayerNorm(d_model)
        self.rel_decoder = TransformerDecoder(rel_decoder_layer, num_rel_decoder_layers, rel_decoder_norm,
    ...
    def forward(self, src, mask, query_embed, rel_query_embed, pos_embed):
        ...
        rel_tgt = torch.zeros_like(rel_query_embed)
        # interaction decoder
        rel_hs = self.rel_decoder(rel_tgt, memory, memory_key_padding_mask=mask,
                                  pos=pos_embed, query_pos=rel_query_embed)
        # insatance decoder
